    ...
    success: function(data) {
      ...
      $.each(arr, function(r, arrKey) {
        ...
        if (
          ...
          obj.name.indexOf(filter) < 0
        ) {
          console.log("Failed filter:" + filter);
          return;
        }
        $("#chartPopupList").append(
          '<li class="list-group-item" >' +
            ...
            "</div>" +
            "<div>" +
            '<a class="btn-primary btn-sm chartAdd" data-key="' +
            arrKey.key +
            '" data-name="' +
            obj.name +
            '" data-scale="' +
            ...
            '<a class="btn-primary btn-sm chartRemove" data-key="' +
            arrKey.key +
            '" data-name="' +
            obj.name +
            '" data-type="' +
            chart +
            '">Hide</a>&nbsp;' +
            '<a class="btn-primary btn-sm chartDelete" data-key="' +
            arrKey.key +
            '" data-name="' +
            obj.name +
            '" data-type="' +
...
function addChartLayer(map, key, type, scale) {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      console.log("chartObj complete: maxZoom:" + data.maxzoom);
      console.log("chartObj complete: minZoom:" + data.minzoom);
